...
Mas https://www.drupal.org/node/2138087 tem um tutorial para usar a RVM no Windows e que funciona.
Seguido o tutorial basta seguir os passos que valem para linux usando o cygwin =).
...
1. Primeiro deploy
	* Certifique-se que o arquivo de deploy _config/deploy.rb_ está corretamente configurado;
 	* Também, certifique-se que o servidor onde será intalado tenha RVM instalado e configurado;
  	* _cap deploy:setup_ Intalará a versão correta do ruby, o gemset e toda a árvore de diretórios;
  	* _cap_ _deploy:migrations_ colocará p código em produção e executara todas as migrações de banco de dados.
...
* testes de unidade e funcionais Ruby
	Está sendo usado rspec (http://rspec.info).
	Para executar $_rake_ _spec_

* Testes de aceitação
	Está sendo usado cucumber (https://cukes.info)
	Para executar $_rake_ _cucumber_

* Testes de unidade JavaScript/CoffeeScript
	Está sendo usado Konacha
	Para executar $_rake_ _konacha:run_
